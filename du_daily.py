...
query = '''SELECT a.`SiteID`, a.`Tipe`, a.`tanggal`, YEAR(a.tanggal) YEAR, WEEK(a.tanggal,4) WEEK, DAY(a.tanggal) DAY,
b.`kabupaten`, b.`kecamatan`, CONCAT(b.`kabupaten`,'-',b.`kecamatan`) kabkec, b.`branch`, b.`cluster`, b.`rtp`,
MAX(a.`RxMax`), MAX(a.`RxAvg`)
FROM `capmon`.`sum_traffic_hourly` a
LEFT JOIN `test`.`dapot_sitename` b
ON a.`SiteID` = b.`site_id`
WHERE a.tanggal BETWEEN "'''+day1+'''" AND "'''+day2+'''" AND a.tipe IN ('S1','IUB') AND b.`region` = 'JABAR'
GROUP BY `SiteID`, `Tipe`, `tanggal`;'''

name_file = 'DU_DAILY_'+day1+'-'+day2+'.csv'
